...
def on_validate_territory_give_user_permissions(self, method=None):
	# for creating new User Permission for each user, if it already doesnt exists
	for u in self.custom_assigned_users:
		user_permission = frappe.db.exists("User Permission", {"user": u.user, "allow": "Territory", "for_value": self.name})

		if user_permission and user_permission != {} and user_permission != None and user_permission != "":
			continue
		else:
			# Creation of User Permission
			new_permission_doc = frappe.new_doc("User Permission")
			new_permission_doc.user = u.user
			new_permission_doc.allow = "Territory"
			new_permission_doc.for_value = self.name
			new_permission_doc.custom_created_by_code = 1
			new_permission_doc.insert()

			# Adding Comment
			comment = frappe.new_doc("Comment")
			comment.comment_type = "Info"
			comment.reference_doctype = "Territory"
			comment.reference_name = self.name
			comment.content = f"""
				User Permission for <b>{u.user}</b> has been <span style="color:green; font-weight:bold">Created</span> for territory <b>{self.name}</b>.
			"""
			comment.save(ignore_permissions=True)

	all_permissions_of_territory = frappe.get_all("User Permission", {"allow": "Territory", "for_value": self.name, "custom_created_by_code": 1})
	
	for p in all_permissions_of_territory:
		perm_doc = frappe.get_doc("User Permission", p.name)
		# If user is removed from assigned user, delete User Permission for it
		if perm_doc.user not in [x.user for x in self.custom_assigned_users]:
			...
			comment.reference_name = self.name
			comment.content = f"""
				User Permission for <b>{perm_doc.user}</b> has been <span style="color:red; font-weight:bold">Deleted</span> for territory <b>{self.name}</b>.
			"""
			comment.save(ignore_permissions=True)
